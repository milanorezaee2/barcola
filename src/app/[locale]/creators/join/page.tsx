...
export default async function JoinPage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const fa = locale === "fa";
  const perks = [
    { icon: BadgePercent, t: fa ? "۷۰٪ سهم فروش" : "70% revenue share", s: fa ? "شفاف، ماهانه، بدون هزینه‌ی عضویت." : "Transparent, monthly, no membership fee." },
    { icon: Globe2, t: fa ? "مخاطب دوزبانه" : "Bilingual audience", s: fa ? "نمایش هم‌زمان به بازار ایران و بین‌الملل." : "Reach both Iranian and international markets." },
    { icon: Palette, t: fa ? "پورتفولیو حرفه‌ای" : "Professional portfolio", s: fa ? "پروفایل، پروژه‌ها و روایت شما در یک‌جا." : "Your profile, projects and story in one place." },
    { icon: ShieldCheck, t: fa ? "حفاظت از لایسنس" : "License protection", s: fa ? "قرارداد و واترمارک برای همه‌ی فایل‌ها." : "Contracts and watermarking for all files." },
  ];
  ...
  return (
    <>
      <PageHero
        ...
        description={fa ? "الگوهایتان را به فضا تبدیل کنید. به جامعه‌ی طراحان رزی آتلیه بپیوندید." : "Turn your patterns into spaces. Join the Rosie Atelier designer community."}
        image={heroImage}
      ...
      <section className="container-x pb-20">
        <div className="grid gap-6 md:grid-cols-4">{perks.map((p, i) => <Reveal key={p.t} delay={i * 60} className="rounded-lg border border-border p-6"><p.icon className="h-5 w-5 text-accent" /><h3 className="mt-4 font-semibold">{p.t}</h3><p className="mt-1.5 text-body-sm text-foreground-secondary">{p.s}</p></Reveal>)}</div>
