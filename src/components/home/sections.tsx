...
/* ------------------------------------------------------------------ */
export function ArtistsSection({ artists, tone = "default" }: { artists: ArtistCardData[]; tone?: "default" | "secondary" }) {
  const { locale, dict } = useLocale();
  return (
    <SectionShell tone={tone}>
...
/* ------------------------------------------------------------------ */
export function PortfoliosSection({ items, title, description, eyebrow, hrefPath, tone = "default" }: { items: PortfolioCardData[]; title: string; description: string; eyebrow: string; hrefPath: string; tone?: "default" | "secondary" }) {
  const { locale, dict } = useLocale();
  return (
    <SectionShell tone={tone}>
      <SectionHeader eyebrow={eyebrow} title={title} description={description} href={href(locale, hrefPath)} hrefLabel={dict.nav.viewAll} />
...
export function StylesSection({ categories, counts }: { categories: Category[]; counts: Record<string, number> }) {
  ...
  /* No row-span anywhere: every row sums to exactly the grid's column count
     (2 on mobile, 6 on md, 12 on lg) so grid-flow-dense can never leave a gap.
     lg: 6+3+3 / 3+3+3+3   ·   md: 6 / 3+3 / 3+3 / 3+3   ·   mobile: full-width stack. */
  const spans = [
  ...
  return (
    <SectionShell tone="secondary">
      ...
      <Reveal className="mt-10">
        <BentoGrid rows="auto-rows-[220px] md:auto-rows-[240px]">
          {cats.map((c, i) => {
            const n = counts[c.id] ?? 0;
            return (
              <StyleCard key={c.id} big={i === 0} href={href(locale, `/styles/${c.slug}`)} title={t(c.name, locale)} description={t(c.description, locale)} image={c.image} className={spans[i]} count={`${locale === "fa" ? faNum(n) : n} ${dict.common.patterns}`} />
            );
          })}
        </BentoGrid>
      </Reveal>
...
export function ExclusiveSection({ products }: { products: ProductCardData[] }) {
  ...
  if (!lead) return null;
  return (
    <SectionShell tone="bleed">
      <div className="container-x">
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0f141c] text-white md:rounded-[28px]">
            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-12">
              ...
              <div className="grid gap-5 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
                {rest.slice(0, 3).map((p, i) => (
                  <Reveal key={p.id} delay={i * 70} className="rounded-xl bg-white p-3 text-foreground dark:bg-surface">
...
/* ------------------------------------------------------------------ */
export function EducationSection({ items, tone = "default" }: { items: EducationCardData[]; tone?: "default" | "secondary" }) {
  const { locale, dict } = useLocale();
  ...
  return (
    <SectionShell tone={tone}>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-12">
        ...
        <Reveal className="lg:col-span-5 rounded-xl border border-border bg-surface p-2 md:p-4">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {items.map((it, i) => (
            <Reveal key={it.path} delay={i * 80}>
              <Link href={href(locale, it.path)} className="group relative flex min-h-[420px] flex-col justify-end overflow-hidden rounded-2xl border border-border p-8 text-white md:min-h-[480px]">
        ...
        {stories.slice(0, 3).map((s, i) => {
          ...
          return (
            <Reveal key={s.id} delay={i * 70}>
              <Link href={href(locale, `/stories/${s.slug}`)} className="group block overflow-hidden rounded-xl border border-border bg-surface p-3 transition-[box-shadow,transform] duration-300 hover:-translate-y-0.5 hover:shadow-medium">
                ...
                <p className="mt-2 line-clamp-2 px-1 pb-1 text-body-sm text-foreground-secondary">{t(s.excerpt, locale)}</p>
              </Link>
            </Reveal>
          );
        })}
...
export function NewsletterSection() {
  ...
  return (
    <SectionShell tone="bleed">
      <div className="container-x">
        <Reveal className="relative overflow-hidden rounded-2xl border border-border bg-surface px-6 py-14 text-center shadow-soft md:rounded-[28px] md:px-12 md:py-20">
